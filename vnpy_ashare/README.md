...
自选池与全 A 股列表默认读取 `data/vnpy_zak.db`；可用 `scripts/export_metadata.py` / `import_metadata.py` 做 CSV 备份。

核心模块：`launcher`（GUI 启动）、`paths`（路径）、`config`（A 股常量）、`models`（StockItem）、`bars`（K 线下载）、`app_db`（元数据库）、`universe`（全市场同步）。

元数据库默认路径：`~/.vntrader/vnpy_zak.db`。UI 在 `ui/`（`main_window`、`quotes_page`、`fonts` 等）。
