...
| 还能不能开仓 | `check_risk_gate` | vnpy-trading | K-04 状态 + 当日亏损 |
...
| vnpy-trading（规划） | check_risk_gate, propose_trading_plan, get_trade_journal |
| vnpy-screening | … + `run_leader_screen`（规划） |
| vnpy-watchlist | … + `get_short_term_watchlist`（规划） |
