...
| `rank/` | 涨幅/成交额等排行定义与取值 |
| `market/` | 市场宽度、北向/环境快照、概览 loader；`emotion_cycle.py`（T-03 **已有**） |
| `misc/` | 持仓异动、涨速基线等辅助指标 |
...
| 页 | 路径 | 职责 |
|----|------|------|
| 板块资金 | `ui/sector_flow/` | 行业/概念资金流向；详情侧栏跳转雷达·龙头 / 主线、龙头选股；成分 **延续快照** + **未来 3 日展望**（批量扫描）；与信号区延续列共用 `stock_continuation` |
