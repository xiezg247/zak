...
```text
守则 | 自选 | 策略 | 市场 | 板块资金 | 雷达 ‖ 选股 ‖ 信息流 ‖ AI 助手
```
...
| `Ctrl+5` | 板块资金 |
| `Ctrl+6` | 雷达 |
| `Ctrl+7` | 选股 Hub（条件选股 + 多因子配方） |
| `Ctrl+8` | 信息流 |
| `Ctrl+9` | AI 助手 |

菜单栏「回测」（弹窗）：`策略回测`（`Ctrl+Shift+8`）· `回测对比`（`Ctrl+Shift+9`，见 [策略回测](./backtest-ux.md)）

菜单栏「后台」（弹窗）：`定时任务`（`Ctrl+Shift+0`）· `数据管理`（`Ctrl+Shift+D`）· `本地数据`（`Ctrl+Shift+L`）

菜单栏「笔记」：`笔记中心`（`Ctrl+Shift+N`，见 [个股笔记](./stock-notes.md)）

全局：`Ctrl+F` 聚焦搜索 · `Ctrl+L` / `⌘L` 显示/隐藏 AI 悬浮球

| 页 | 说明 |
|----|------|
| 守则 | Playbook Tab（默认买卖）规则表格 |
