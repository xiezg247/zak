...
个人 A 股现货量化终端：选股 → 看盘 → 回测 → AI 解读。交易风格以**极致短线为主、中线为辅**，体系化需求见 [交易体系需求](./trading-system.md)；日路径见 [盘中工作流](./intraday-workflow.md)。
...
```text
自选 | 市场 | 板块资金 | 雷达 | 本地 | 选股 | AI 助手 | 策略回测 | 回测对比
```
...
| 策略回测 | 单标的回测 |
| 回测对比 | 批量回测批次对比 |
| 定时任务 | 下载、行情采集、universe 同步、盘中多因子 |
| 数据管理 | vnpy K 线维护 |
