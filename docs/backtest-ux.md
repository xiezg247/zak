...
**批量回测**：`batch_backtest_flow.py` 后台逐只回测 → 落库 `batch_id` → 跳转回测对比页。需本地已有日 K。
...
| 文件 | 职责 |
|------|------|
| `events.py` | `EVENT_OPEN_BACKTEST`、`BacktestRequest` |
| `ui/batch_backtest_flow.py` | 批量回测 |
| `ui/batch_backtest_page.py` | 回测对比页 |
| `backtest/run_store.py` | `backtest_runs` |
| `services/backtest_service.py` | 摘要落库与 context |
| `ai/backtest_context.py` | 回测页 AI 上下文 |
