...
> **纪律**：只做计划内交易；计划外操作须可标记、可统计。复盘闭环见 [盘中工作流 §3.3](./intraday-workflow.md)。笔记流水（已有）与结构化流水（待建）**并存**。
...
| 入口 | 说明 |
|------|------|
| 雷达「生成次日计划」 | AI 草稿 → 确认对话框 |
| 自选工具栏「今日计划」 | 侧边抽屉只读/编辑 |
| 笔记中心 Tab「计划」 | 历史计划列表 |
...
| 仅笔记流水 | 不替代 trade_journal；可一键「从笔记导入」 |
