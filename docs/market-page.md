...
| 成交额汇总 | **已有** | < 1 万亿橙色提示 |
| 涨幅 / 跌幅 / 换手排行 | **已有** | 人气扫描 |
| 涨停榜 `limit_up` | **已有** | 10cm 主池；表格含「连板」列 |
| 连板榜 `limit_times` | **已有** | 侧栏「连板榜」；按连板数排序，替代「涨停榜内分层筛选」 |
...
连板梯队分层见侧栏 **连板榜**（`quotes/rank/rank_catalog.py` `limit_times`）与雷达 D-01「发现·连板梯队」；本页涨停榜表格亦含「连板」列。
