...
| 模块 | 说明 |
|------|------|
| `vnpy_ashare/storage/connection.py` | `connect()` → PG `app` schema |
| `vnpy_ashare/storage/cache/db_session.py` | Cache schema 会话 |
| `vnpy_common/storage/session.py` | `connect_app` / `cache_session` / `chat_session` |
| `vnpy_ashare/storage/repositories/*.py` | SQLAlchemy Core + raw SQL（`%s` 占位符） |
...
- [x] `connection.py`、`chat/store.py` 切 PG
- [x] repository / store / cache 切 PG
