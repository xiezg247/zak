...
zak 终端已有「条件选股」「多因子配方（Recipe）」「AI 对话选股」三条路径，但盘中能力薄弱：
...
- `RECIPE_EXCLUDE_SUSPENDED`：`0` 关闭停牌过滤
- `RECIPE_EXCLUDE_NEW_LISTING` / `RECIPE_MIN_LISTING_DAYS`：新股过滤
- `RECIPE_EXCLUDE_LIMIT_BOARD`：`0` 关闭涨跌停过滤
- `RECIPE_MIN_TOTAL_MV_WAN`：总市值下限（万元）
...
选股页合并为单一导航项「选股」（`ScreenerHubPageWidget`），内嵌两个 Tab：

| Tab | 说明 |
|-----|------|
| 条件选股 | preset / 自定义条件；`ScreeningDataStatusBar` 展示数据源与快照年龄，交易时段可一键刷新行情 |
| 多因子配方 | Recipe 运行与历史收件箱；左侧 `[盘中]` / `[盘后]` 过滤 |

两 Tab 共用结果洞察（`ScreenerResultInsights`）：

- 文本摘要：较上次 run diff（`run_diff.py`）
- 行业分布：`SectorDistributionPanel` 条形占比 + 涨跌着色（Top 6，数据来自 `sector_summary.py`）

雷达页权重变更后，相关卡片触发全量重算（`OUTLOOK_FORCE_RECOMPUTE_CARD_IDS`）。
...
- [x] 结果区行业分布面板（`sector_distribution_panel.py` + `ScreenerResultInsights`）
