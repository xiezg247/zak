...
|----|------|---------|----------|------|
| R-01 | `ultra_short_limit` | intraday | 涨停 + 连板 + 板块强度 + 换手 | 极致短线主池（**已有**） |
...
| `propose_recipe` | NL → 配方草案；复杂/自定义时调用，**客户端确认后执行** |
...
| 已保存方案 / 模糊条件 | `propose_screening`（确认后执行） |
| 自定义多因子配方 | `propose_recipe`（确认后执行） |
