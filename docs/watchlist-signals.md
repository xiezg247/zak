...
工具栏 **「策略/持仓 · 信号 n · 持仓 n」** 主控工作区显隐（见 [watchlist-ui.md §2.4](./watchlist-ui.md#24-策略持仓工作区)）；**「加入信号区」** 在登记预设等场景可见。策略快/慢线、列配置、巡检间隔、刷新、移出、清空在信号区 header。
...
| 延续 / 展望 | `services/signals/stock_continuation.py` | 价量 + 资金 + 板块环境 → `continuation_pattern`、`outlook_compact` |
| 列定义 | `config/preferences/signal_panel_columns.py` | 固定列 + 可选列；QSettings 持久化 |
...
| `continuation_pattern` | 延续模式（价量 + 个股资金序列 + 所属板块环境） |
| `outlook_compact` | 未来 3 个交易日统计展望（非确定性，含 `STOCK_CONTINUATION_DISCLAIMER`） |

辅助函数：`signal_missing_kline()`、`signal_as_of_stale()`、`signal_row_sort_key()`（买 > 观望 > 卖 > —，同档按强度、代码）。

延续列由 `build_stock_continuation_snapshot()` 计算，与板块资金页成分展望共用 `classify_flow_pattern_values` / 资金序列逻辑；板块页批量扫描见 [架构说明 §板块资金与雷达](./architecture.md#板块资金与雷达)。
...
| 场景 | 行为 |
|------|------|
| stale 巡检（交易时段，默认 30 分钟，可配） | 仅 `_symbols_needing_refresh`；与 3 秒行情定时器分离 |
| 全局 Scheduler「策略信号磁盘预热」 | 收盘后为信号区/持仓名单写入磁盘 cache（默认关闭，建议接在日 K 补全之后） |
| 页面激活（交易时段） | 延迟约 0.8s 补一次 stale 巡检 |
| 手动「刷新」 | `force=True`，重算信号区全部标的 |
| 参数变更 | `invalidate_cache()` + `force=True` |
| 日 K 下载/补缺口 | 仅 `refresh_symbols` 受影响标的 |
| 名单变更 | 移除不在名单的 cache 项；新增标的 `force=True` |
| 禁用「启用信号」 | 停止 Worker 请求，仍展示已有 cache（只读） |
...
**默认可选列**（`DEFAULT_VISIBLE_OPTIONAL_KEYS`）：信号、信号天、量比、快慢距%、支撑锚点、阻力锚点、参考买价、参考卖价、距买价%、距卖价%、强度、相对300%。

**额外可选列**（默认隐藏，header「列」菜单勾选）：延续模式（`continuation_pattern`）、未来3日（`outlook_compact`）。

列配置持久化：`QSettings` key `watchlist/signal_panel/columns`；定义见 `signal_panel_columns.py`。
...
```text
stale 巡检 Timer（交易时段，默认 30 分钟，header 可配）→ refresh(force=False)
...
| 文件 | 覆盖 |
|------|------|
| `tests/ashare/ui/test_watchlist_signals.py` | settings、stale/missing kline、排序、磁盘 cache、列配置 |
| `tests/ashare/test_stock_continuation.py` | 延续模式与未来 3 日展望 |
| `tests/ashare/jobs/test_watchlist_strategy_prewarm.py` | 收盘磁盘预热 Job |
