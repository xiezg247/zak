...
- 本地检查：`uv run ruff check .` / `uv run ruff format .`
...
- 行情 Worker：`ui/quotes/workers/`（K 线 / universe 经 `bar_access`）
- 选股 Worker：`ui/workers/screener_workers.py`
- 兼容入口 `ui/worker.py` 仅 re-export，新代码勿再增加依赖
...
- 图表 Tab 索引等共享常量放 `ui/chart_tab_indices.py`，勿让 controller 从 `chart_panel` 反向 import
