...
- 类型检查：**mypy**（各 `packages/*/pyproject.toml` → `[tool.mypy]`，聚合见 [mypy 静态类型检查](./mypy.md)）
- 本地检查：`uv run ruff check .` / `uv run ruff format .` / `bash scripts/mypy-check.sh`
...
- **允许函数内 import 的场景**（须在行尾或上一行简短注明原因）：
  ...
  - 已验证的**循环依赖**（如 `engine_access.get_ashare_engine`）
