...
class TradeCalendarJobTests(unittest.TestCase):
    def test_skips_when_token_missing(self) -> None:
        with patch(
            "vnpy_ashare.jobs.sync.trade_calendar.get_tushare_pro",
    ...
    def test_reports_sync_count(self) -> None:
        with patch("vnpy_ashare.jobs.sync.trade_calendar.get_tushare_pro", return_value=object()):
            with patch(
                "vnpy_ashare.jobs.sync.trade_calendar.sync_trade_calendar",
                return_value=1200,
            ):
                result = sync_trade_calendar_job()
