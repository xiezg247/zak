...
class StockIndustryJobTests(unittest.TestCase):
    def test_skips_when_token_missing(self) -> None:
        with patch(
            "vnpy_ashare.jobs.stock_industry.get_tushare_pro",
    ...
    def test_reports_industry_count(self) -> None:
        with patch("vnpy_ashare.jobs.stock_industry.get_tushare_pro", return_value=object()):
            with patch(
                "vnpy_ashare.jobs.stock_industry.fetch_stock_basic_snapshot",
