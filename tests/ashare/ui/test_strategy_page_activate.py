...
def test_radar_activate_defers_heavy_refresh() -> None:
    from vnpy_ashare.ui.quotes.page.session_lifecycle import activate_quotes_page
    ...
    with patch(
        "vnpy_ashare.ui.quotes.page.session_lifecycle._deferred_radar_activate_frame1",
    ) as frame1:
        with patch("vnpy_ashare.ui.quotes.page.session_lifecycle.QtCore.QTimer.singleShot", side_effect=_capture_timer):
            activate_quotes_page(page)

        assert page._active is True
        page._radar_controller.activate_heavy.assert_not_called()
        assert len(timers) == 1
        timers[0]()
        frame1.assert_called_once_with(page)


def test_radar_activate_frame1_ends_tab_loading() -> None:
    from vnpy_ashare.ui.quotes.page.session_lifecycle import _deferred_radar_activate_frame1
