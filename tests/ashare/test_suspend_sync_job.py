...
class SuspendSyncJobTests(unittest.TestCase):
    def test_skips_when_token_missing(self) -> None:
        with patch(
            "vnpy_ashare.jobs.suspend_sync.get_tushare_pro",
    ...
    def test_reports_sync_count(self) -> None:
        with patch("vnpy_ashare.jobs.suspend_sync.get_tushare_pro", return_value=object()):
            with patch(
                "vnpy_ashare.jobs.suspend_sync.sync_suspend_recent",
