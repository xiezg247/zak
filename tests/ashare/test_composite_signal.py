...
def _series(count: int, *, start: float = 10.0, step: float = 0.3) -> tuple[list[float], list[date], list[float], list[float]]:
    ...
    return closes, dates, highs, volumes


def test_composite_payload_includes_strength_and_reason():
    closes, dates, highs, volumes = _series(80)
...
def test_composite_buy_on_bull_alignment_and_volume():
    closes, dates, highs, volumes = _series(80, step=0.5)
    # 放大近期成交量
...
def test_ref_prices_use_high_for_sell():
    closes, dates, highs, volumes = _series(30)
    payload = build_composite_signal_payload(
        closes,
        dates,
        vt_symbol="600000.SSE",
