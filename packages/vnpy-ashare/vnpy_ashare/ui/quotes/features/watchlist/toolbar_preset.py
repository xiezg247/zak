...
def _sync_emotion_chips(page: WatchlistHost, *, show_in_toolbar: bool) -> None:
...
def create_emotion_more_buttons(page: WatchlistHost) -> list[tuple[str, QtWidgets.QPushButton]]:
    """创建供「更多」菜单调用的情绪周期代理按钮（默认隐藏，由预设切换显隐）。"""
    ...
    return [(_EMOTION_MORE_LABEL, emotion_btn)]
...
def apply_toolbar_for_preset(page: WatchlistHost, preset_id: LayoutPresetId) -> None:
    ...
    if getattr(page, "_watchlist_feature", None) is not None:
        _sync_emotion_chips(page, show_in_toolbar=False)
        _set_more_menu_action_visible(page, _EMOTION_MORE_LABEL, spec.show_emotion_chips)
        return
    ...
    _sync_emotion_chips(page, show_in_toolbar=spec.show_emotion_chips)
