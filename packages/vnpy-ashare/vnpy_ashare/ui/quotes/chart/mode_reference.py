...
_LIMIT_BOARD_STRATEGIES = frozenset({"AshareLimitBoardStrategy", "AshareLimitBoardMinuteStrategy"})
_HALFWAY_STRATEGIES = frozenset({"AshareIntradayBreakoutStrategy", "AshareIntradayBreakoutMinuteStrategy"})
_PULLBACK_STRATEGIES = frozenset({"AsharePullbackStrategy", "AsharePullbackMinuteStrategy"})
...
def resolve_intraday_mode_kind(strategy_id: str | None) -> IntradayModeKind:
    ...
    if sid in _LIMIT_BOARD_STRATEGIES:
        return "limit_board"
    if sid in _HALFWAY_STRATEGIES:
        return "halfway"
    if sid in _PULLBACK_STRATEGIES:
        return "pullback"
    return "none"
