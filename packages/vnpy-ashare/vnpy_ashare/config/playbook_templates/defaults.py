...
| 原则 | 靠盈亏比取胜，靠纪律存活 |
| 止损 | 单笔止损与隔日铁则优先于「再等等」 |
| 浮亏 | 浮亏达警戒线须对照 Playbook 复盘，不扛单死等 |
| 提示 | 情绪退潮 / 计划外持仓 → 添加持仓时会有提示 |
"""
...
| 阶段 | 盘面信号 | 建议仓位 | 动作 |
|------|----------|----------|------|
| 退潮 | 龙头跌停、连板批量断板 | **0%** | 无条件空仓 |
| 分歧 | 龙头炸板、跟风掉队 | ≤30% | 仅核心低吸 |
| 发酵/高潮 | 梯队完整 | 60–80% | 龙头/前排 |
| 启动 | 出现 3 板以上龙头 | 30–50% | 试错首板/二板 |
| 冰点 | 连板高度 ≤2、跌停偏多 | 0–10% | 极小试错或空仓 |
...
| 维度 | 规则 |
|------|------|
| 标的 | 只做有辨识度的人气股；拒绝杂毛跟风 |
| 主池 | 极致短线主池：涨停 + 连板 + 板块强度 + 换手 |
| 硬过滤 | 排除 ST / 停牌；成交额与市值带按 Profile 镜像区为准 |
| 流程 | 雷达共振 / 选股结果 → 先入自选，再进信号区（上限 10） |
"""
...
| 场景 | 信号 | 动作 |
|------|------|------|
| 情绪退潮 / 冰点 | 连板断档、跌停增多 | 减少新开，以观察为主 |
| 情绪启动 / 发酵 | 题材梯队成形 | 可参与短线波段突破 |
| 大盘走弱 | 5 日线拐头向下 | 降低频率，硬过滤偏保守 |
"""
...
| 维度 | 规则 |
|------|------|
| 风格 | 短线放量突破为主；关注题材发酵与龙头趋势延续 |
| 硬过滤 | 默认「均衡」：流动性 + 市值带 + 排除 ST |
| 配合 | 雷达「未来·展望」与信号区配合，不追杂毛 |
"""
