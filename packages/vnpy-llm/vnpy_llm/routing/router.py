...
TOOL_GROUPS: dict[IntentCategory, frozenset[str]] = {
    ...
    "radar": frozenset(
        {
            "get_radar_snapshot",
            "get_leader_pick_snapshot",
            "run_leader_screen",
            "run_short_term_screen",
            "get_emotion_cycle",
            "get_screening_context",
            "explain_screening_run",
        }
...
- market：大盘环境、情绪周期、择时、短线能不能做、风控闸、连板结构、行业轮动、龙头环境
- diagnosis：个股综合诊断、基本面与资金面
- screening：选股、筛选、涨幅榜、换手率等条件选股
- radar：雷达盘面、龙头、共振、极致短线抓票（vnpy-radar）
...
screening 补充：短线游资/题材活跃 → recipe_id=intraday_multi；极致短线雷达统一 → ultra_short_unified 或 vnpy-radar run_short_term_screen；中线波段 → recipe_id=post_close_multi；
...
def _infer_market_enrichment(
    ...
    if category == "market":
        return MarketEnrichment(fear_greed="highlight", reasoning="市场择时/环境评估")
...
def _keyword_fallback(user_text: str, page: str) -> IntentAnalysis | None:
    ...
    if any(k in text for k in _MARKET_KEYWORDS) or text.startswith("请评估今日 A 股极致短线"):
        return _with_market("market")
...
def _resolve_route_tools(
    ...
    *,
    ...
    if analysis.route.confidence == "low" and category not in ("general", "team_analysis", "market"):
        tools: list[dict[str, Any]] = []
    else:
...
_MARKET_COMMAND = re.compile(r"^/market(?:\s+(intraday|environment))?\s*$", re.IGNORECASE)


def normalize_market_command(user_text: str) -> str | None:
    """将 /market 命令展开为自然语言，供路由分类使用。"""
    m = _MARKET_COMMAND.match(user_text.strip())
    if not m:
        return None
    focus = (m.group(1) or "intraday").lower()
    if focus == "environment":
        return "请解读当前 A 股市场环境（大盘强弱、涨跌广度、涨跌停、成交额、恐贪指数、北向资金）"
    return "请评估今日 A 股极致短线交易环境：是否适合新开仓、连板结构与仓位建议、主线行业与需规避的风险"
